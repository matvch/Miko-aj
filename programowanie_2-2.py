...
Original file is located at
    https://colab.research.google.com/drive/1NsfHTxtWvuDqys0D3pNRCW2dghfJVjYP
"""
...
# --- 7. Zapisywanie SVG do pliku (dla GitHub) ---
# Dodajemy możliwość zapisania wygenerowanego SVG do pliku, co ułatwi osadzenie go na GitHubie lub w innych dokumentach.
file_name = "moj_mikolaj_static.svg"
try:
    with open(file_name, "w") as f:
        f.write(svg_code)
    st.success(f"🎅 Mikołaj został zapisany jako plik: {file_name}. Możesz go teraz pobrać i umieścić na GitHubie!")
    # Optionally, provide a direct download link for the saved file as well
    st.markdown(f'<a href="data:image/svg+xml;base64,{b64}" download="{file_name}">📥 Pobierz {file_name}</a>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Wystąpił błąd podczas zapisywania pliku SVG: {e}")